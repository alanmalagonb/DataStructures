...
Pila ProcesarOperador(Pila ops,Elem pos[],Elem op){
    ...
    else if(EsMayor(Precedencia(op),Precedencia(top(ops))))
        return ops=push(op,pos);
    else{
        while(!EsParIzq(top(ops))&&!EsMayor(Precedencia(op),Precedencia(top(ops)))){
            pos[i++]=top(ops);
            ops=pop(ops);
        };
        return ops=push(op,pos);
...
char* CopiarPilaApos(Pila ops,Elem pos[]){
    int i=0;
    while(!isempty(ops)){
        pos[i]=top(ops);
        ops=pop(ops);
